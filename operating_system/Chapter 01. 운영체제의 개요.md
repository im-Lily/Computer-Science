...
2. 운영체제의 필요성
    1. 성능 및 효율의 향상
       ...
       <br><br>
    2. 자원 관리 역할
        - 응용 프로그램을 차지하려는 다양한 하드웨어와 응용 프로그램 간 자원 경쟁 - 자원을 관리하는 중재 역할
...
5. 분산 시스템 (1970년대 후반) - 개인용 컴퓨터와 인터넷 등장
   - 네트워크상에 분산되어 있는 여러 컴퓨터로 작업 처리 및 결과 상호 교환
   <br><br>
6. 클라이언트/서버 시스템 (1990년대 ~ 현재) - 웹 시스템
   - 클라이언트가 서버에 작업을 요청하면 서버가 해당 작업을 처리하는 구조
   - 모든 요청이 서버로 집중될 경우 서버 과부하 발생
   <br><br>
7. P2P 시스템 (2000년대 초반 ~ 현재) - 메신저, 파일 공유
   - 서버를 거치지 않고 사용자와 사용자를 직접 연결하여 서버 과부하 문제 해결
   <br><br>
8. 기타 컴퓨팅 환경 (2000년대 초반 ~ 현재) 
   - 그리드 컴퓨팅 : 필요한 컴퓨팅 자원을 구매하여 사용
   - 클라우드 컴퓨팅 : 언제 어디서나 응용 프로그램과 데이터 자유롭게 사용 가능
   - 사물 인터넷 : 사물에 내장된 센서와 통신 기능과 인터넷 연결
   <br><br>
...
2. 시스템 호출과 디바이스 드라이버
   1. **시스템 호출**
        ...
        => **시스템 호출**이라는 인터페이스를 이용하여 접근 가능
        - 시스템 호출을 거치지 않고서는 커널 진입 불가능
        - 시스템 호출은 커널이 제공하는 시스템 관련 서비스를 모아놓은 것으로 함수 형태로 제공됨   
        =>print(), write(), read()
      <br><br>
   2. **디바이스 드라이버** 
      - 하드웨어 인터페이스 담당
      - 커널이 제공하는 드라이버와 하드웨어 제작자가 제공하는 드라이버가 있음
      - 마우스, 키보드와 같은 디바이스 드라이버는 커널에 포함되어 있어 컴퓨터에 꽂기만 하면 작동됨
      - 그래픽카드, 프린터와 같이 크기가 크고 복잡한 디바이스 드라이버는 사용자가 직접 설치해야함
      <br><br>
3. 커널의 구성 
   - 커널의 역할 : 프로세스 관리, 메모리 관리, 파일 시스템 관리, 입출력 관리, 프로세스 간 통신(IPC)
   <br><br>
   1. **단일형 구조 커널** 
       - 커널의 핵심 기능을 구현하는 모듈 하나로 구성
       - 모듈 간의 통신 비용이 감소하여 효율적인 운영이 가능하나 운영체제의 여러 기능이 서로 연결되어 있어 기능상의 작은 결함 시스템 전체로 확산 가능
       - 모든 모듈이 하나로 묶여 있어 수정이 어렵고 이식성이 낮음
      <br><br>
   2. **게층형 구조 커널**
       - 비슷한 기능을 가진 모듈을 묶어서 하나의 계층으로 만들고 계층 간의 통신을 통해 운영체제 구현
       - 오류 발생 시 해당 계층만 따로 수정가능하여 디버깅이 용이함
      <br><br>
   3. **마이크로 구조 커널**
      - 프로세스 관리, 메모리 관리, 프로세스 간 통신 관리 등 가장 기본적인 기능만 제공하고 나머지 기능은 사용자 영역에 구현
      - 각 모듈은 독립적으로 작동하기 때문에 하나의 모듈이 실패하더라도 전체 운영체제에 영향을 미치지 않고 이식성이 높음
      <br><br>
